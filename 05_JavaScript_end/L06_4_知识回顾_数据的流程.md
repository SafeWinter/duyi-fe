...
for(var i = 0, len = students.length; i < len; i++) {
    var st = students[i];
    if(st.sex === '女') {
        girls.push({st});  // girls.push({...st});
    }
}
console.log(girls);

// 得到所有年龄在25岁以下的女生（新数组）
var ageLT25 = students.filter(s => s.age < 25);
console.log(ageLT25);

// 得到所有姓陈的学生（新数组）
var chens = students.filter(s => s.slice(0, 1) === '陈');
console.log(chens);

// 得到所有电话号码以1结尾的学生（新数组）
var telEndsWith1 = students.filter(s => s.tel.substr(-1) === '1');
console.log(telEndsWith1);
...
// 得到所有女生的姓名数组（新数组）
var girlNames = students.filter(s => s.sex === '女').map(s => s.name);
console.log(girlNames);

// 得到所有女生的姓名和电话号码 [ {name:'monica', tel:'18122223333'} ]
var girlsNameTel = students.filter(s => s.sex === '女').map(({name, tel}) => ({name, tel}));
console.log(girlsNameTel);

// 得到所有学生的年龄的总和
var ageSum = students.reduce((acc, e) => acc + e.age, 0);  // 1299

// 得到所有学生的平均年龄
var ageAvg = students.reduce((acc, e) => acc + e.age, 0) / students.length;  // 21.65

// 得到一个对象： {name:['张三', '李四', ...], age: [17, 25, ...]}
var arrNameAge = students.reduce((acc, {name, age}) => {
    acc.name.push(name);
    acc.age.push(age);
    return acc;
}, {name: [], age: []});
// 点评：解构赋值部分不能再加小括号，写作：({name, age})

// 找到id为796997的学生对象
var st796997 = students.find(s => s.id === 796997);
// {id: 796997, name: '郝霞', sex: '女', age: 29, address: '辽宁省 锦州市', …}

// 是否包含年龄大于28岁的男生
var hasBoyAgeLT28 = students.some(s => s.age > 28) ? '是' : '否';
console.log(hasBoyAgeLT28);  // 是

// 是否所有的女生年龄都在28岁以内
// Solution1:
var result = true;
for(var i = 0, len = students.length; i < len; i++) {
    result = result && (students[i].age <= 28);
}
console.log('所有的女生年龄都在28岁以内：', result ? '是' : '否');
// Solution2:
var result = true;
...
console.log('所有的女生年龄都在28岁以内：', (result && i > len) ? '是' : '否');
// DIY
var girlAgeAllLE28 = students.every(s => s.age <= 28) ? '是' : '否';
console.log(girlAgeAllLE28);  // 否
```
