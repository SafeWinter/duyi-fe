...
`JavaScript` 能不能玩得很溜，关键在于能不能玩得懂回调函数。
...
const myFilter = filter(students);

// 得到所有女生
const isGirl = s => s.sex === '女';
const r1 = myFilter(isGirl);

// 得到所有20岁以下的女生
const girlLT20 = s => isGirl(s) && s.age < 20;
const r2 = myFilter(girlLT20);

// 得到所有姓赵的同学
const surnameZhao = s => s.name.startsWith('赵');
const r3 = myFilter(surnameZhao);

// 得到所有居住在黑龙江的女生
const girlInHLJ = s => isGirl(s) && s.address.includes('黑龙江');
const r4 = myFilter(girlInHLJ);
```
