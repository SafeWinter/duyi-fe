...
// 得到所有电话号码以1结尾的学生
const endsWith1 = students.filter(s => s.tel.endsWith('1'));
lg(endsWith1); // [{...}, {...}, {...}]
...
const girlsNamesAndTels = students
  .filter(s => s.sex === '女')
  .map(s => ({ name: s.name, tel: s.tel }));
lg(girlsNamesAndTels); // [{name:'monica', tel:'18122223333'}, {...}, {...}]

// 将每个学生变为一个 li 元素字符串，元素内容为学生姓名，拼接所有元素，形成一个长字符串 <li>张三</li><li>李四</li>...
const html = students.map(s => `<li>${s.name}</li>`).join('');
lg(html); // <li>梁平</li><li>邱杰</li><li>王超</li>...
