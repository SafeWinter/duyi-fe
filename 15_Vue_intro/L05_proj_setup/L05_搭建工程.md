...
> [!note]
>
> **（20260105）实测组件 `Avatar.vue` 的增补笔记**
>
> 1. 将传入的 `size` 的值通过 `computed` 转换成像素后，生成新的样式对象 `avatarStyle` 放入模板：
>
>    ```js
>    /*<div class="avatar">
>      <img :src="url" class="avatar-img" :style="avatarStyle">
>    </div>*/
>    export default {
>      // -- snip --
>      computed: {
...
>    ```
>
> 2. 回忆 `img` 元素的 `src` 属性变为响应式数据后的写法：必须 `import` 导入，否则图片路径丢失（必须由 `Webpack` 打包）；
>
> 3. 解决 `vuejs devtools` 调试工具的 `Vue` 标签不显示的问题：进入扩展详情，启用无痕浏览，然后 <kbd>Ctrl</kbd>+<kbd>Shift</kbd>+<kbd>N</kbd> 打开无痕模式，按 <kbd>F12</kbd> 即可看到 `Vue` 标签：
>
>    ![Restore the Vue.js Devtools extension](../assets/5.4.png)

---

[^1]: 本节示例代码，详见 `15_Vue_intro\proj1_my_site`，参考 `Git` 标签：`L05_my-site`
